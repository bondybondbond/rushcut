...
description: "Cold-context, read-only reviewer for a single just-built RushCut implementation step (UI screen/component or pipeline change). Invoked by rushcut-dev-plan Step 7 via the Agent tool (run_in_background: true) after each step with a defined acceptance check is implemented. Given only a git diff, the review target (screen/route for UI, or function/module + log/artifact paths for pipeline), and the acceptance checks defined in Step 6 — never the implementation rationale. Assumes each acceptance check is unmet until proven otherwise, then returns one schema-shaped verdict (pass/fail/blocked). Never invoked directly by the user."
