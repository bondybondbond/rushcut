...
1. **Check Chrome is connected — and launch it yourself if not.** Call `list_connected_browsers` / `mcp__claude-in-chrome__tabs_context_mcp`.
   - **If nothing is connected, launch the automation Chrome yourself via PowerShell** (Profile 4 is the specific profile with the Claude-in-Chrome extension installed and signed in — a default/other profile will NOT have it), then wait ~5s and re-call `list_connected_browsers` **once**:
     ```powershell
     Start-Process "C:\Program Files\Google\Chrome\Application\chrome.exe" -ArgumentList '--remote-debugging-port=9222','--user-data-dir=C:\Users\Manasak\AppData\Local\Google\Chrome\User Data\Profile 4','--no-first-run','--no-default-browser-check'
     ```
   - **If it connects after that** — proceed with Gate 3 normally.
   - **If it still does not connect, first check what holds CDP port 9222:** `(Invoke-WebRequest -UseBasicParsing http://127.0.0.1:9222/json/version).Content`. If the `Browser` field reads `Edg/…` (a WebView2 — e.g. a running `rushcut.exe` launched with `--remote-debugging-port=9222`), the port is squatted and Chrome cannot bind it. Do **not** kill `rushcut.exe` from inside this agent — return to CC with: *"Gate 3 blocked: CDP port 9222 is held by a WebView2/rushcut.exe instance; the automation Chrome cannot bind it. CC must free 9222 (stop rushcut.exe) then re-spawn Gate 3."*
   - **Only if 9222 is genuinely free and Chrome still will not connect** after the launch + one retry: fall back to Claude `WebSearch` for Query 1/2 as the documented degraded path, and flag the gap explicitly in the Gate 3 scratch file (note that the launch-and-retry was attempted and failed).
...
**Reading the response — a `get_page_text`/`read_page` capture is MANDATORY after every submit, even when you also use the clipboard.** The mechanical Gate 3 proof (`.claude/hooks/lib/transcript.js` `countGateCycles`) only credits a query cycle if a `mcp__claude-in-chrome__get_page_text` or `mcp__claude-in-chrome__read_page` call returns **≥300 characters of genuinely-new response text** after that query's submit. A clipboard read (`read_clipboard`) does NOT count toward the proof, no matter how complete its content — so Gate 3 will fail the hook even though real research happened (confirmed live 2026-08-28). Procedure per query:
1. After submitting, wait ~15-20s for the full response to render.
2. Scroll the response body into view if needed, then call `read_page` (accessibility tree — most reliable for long ChatGPT replies) or `get_page_text`. Re-read once if it looks short/incomplete. This capture is what proves the cycle — do not skip it or substitute clipboard for it.
3. ONLY if that page-text capture is still incomplete/garbled/truncated for your own analysis needs, additionally use the clipboard mechanism (`request_access` with `clipboardRead`/`clipboardWrite` grants, click ChatGPT's copy-response icon under the reply, `read_clipboard`) to get the full text for the findings table. Note in your response that you used the clipboard supplement and why — but the page-text capture from step 2 must still have happened first.
...
**Chrome-unavailable fallback for Gate 3 — only after Setup step 1's launch-and-retry has actually been tried and failed.** Do not fall straight to WebSearch on a first "not connected" result; you must run the `Start-Process` Chrome launch (Profile 4) and re-check once first, and — if 9222 is held by a WebView2/rushcut.exe — hand the port-collision case back to CC rather than degrading. Only when the launch genuinely fails against a free port do you run Query 1/2 as Claude `WebSearch` instead (no other agent exists to fall back to now). Explicitly log in the Gate 3 scratch file that the launch-and-retry was attempted and failed, and that the round is degraded. (Note: this is a different case from the quota pause in Setup step 6 — quota-paused means Chrome IS working and ChatGPT IS reachable, just rate-limited; that case is handled by switching chats, not by falling back to WebSearch.)
