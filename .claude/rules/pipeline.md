...
- **Source FPS mismatch causes visible stutter on constant-motion clips** — the pipeline hardcodes `-r 25 -fps_mode cfr` in normalise.py and proxy.py. DJI Osmo Pocket 3 shoots at 29.97fps (30000/1001). Converting 29.97→25 drops ~1 frame every 5 output frames (ratio 1.1988); on pans this creates a visible pulse every ~200ms. Existing 25fps proxies have the stutter baked in. **Fix (Batch Q2, deferred):** detect target_fps from source clips (ffprobe r_frame_rate, round 29.97→30, 25.0→25), pass through manifest, use in normalise + proxy. Add FPS check to proxy reuse gate — reject proxies whose FPS ≠ target_fps. See `docs/BATCH-Q2-FPS-STUTTER.md`.
