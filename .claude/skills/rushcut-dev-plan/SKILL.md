...
1. **Pre-check (UI steps only) — judge IPC/DB-dependency before invoking the reviewer.** `rushcut-qa-reviewer` uses `preview_*` MCP tools exclusively, which always drive Claude's own browser against the bare Vite dev server — they never attach to a real Tauri binary's IPC layer (confirmed #90, #93; no supported attach-to-external-CDP mechanism exists). If the screen being reviewed requires `invoke()`/DB-backed data (a real project id, thumbnails/waveform/proxy state from `scan.py`, or any Tauri command result), **skip the reviewer invocation** and record the check as: *"IPC-dependent — not verifiable via preview; requires WDIO or manual user verification."* Only dispatch to the reviewer for backend-independent UI checks (static shell, layout, CSS, routes needing no real data).
2. For any step that clears the pre-check (backend-independent UI step, or any pipeline/backend step), immediately after finishing it invoke the `rushcut-qa-reviewer` subagent via the Agent tool with `run_in_background: true`. Pass it only:
...
- **Grep before flagging a `[TRAP]` — mandatory, not advisory:** before writing a `[TRAP]:` line (or re-deriving a fix for an unexpected `gh`/tooling/pipeline command failure), run `grep -n "<core command or keyword>" docs/LEARNINGS.md` first. Match found → the pattern is already documented; don't re-flag it, just follow the existing entry. No match → it's genuinely new, flag it as usual. This exists because the same `gh project item-list` truncation lesson was independently re-discovered three times despite already being written down — the fix is catching it before it's re-learned, not cleaning up duplicates after.
