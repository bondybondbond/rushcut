...
- `rc-trans-preview-a` → `background: #1f4a63` (deep teal-blue); `rc-trans-preview-b` → `background: #63304a` (deep plum). Both are flex-centred and contain `<span className="rc-trans-panel-label">A|B</span>` — big white letter (13px in the rail, `clamp(2.5rem, 11vw, 5rem)` in the centre) so the mechanic is unmistakable even though the panels are abstract.
- `style={{ animationName: ANIM_KEYS[value].a/b }}` — **the keyframe NAME only, never the `animation` shorthand.** Duration / iteration-count / timing-function / `animation-play-state: paused` are CSS longhands on `.rc-trans-preview-a/-b`. Setting the shorthand from JS resets `animation-play-state` to `running` and defeats the gate (#192).
- Wrapper: `relative w-16 h-10 bg-black overflow-hidden` (rail) / `rc-trans-centre-preview relative h-56 … bg-black` (centre) — `bg-black` still required so the mid-dip frame shows pure black.
- **Selected card:** `rc-trans-card--selected border-[#99B3FF]` (border only — no animation change).
- `@media (prefers-reduced-motion: reduce)` freezes the centre preview at the same 40% still; the rail is already static so it is untouched.
- **`data-testid="chip-transition-{value}"`** on the `<button>` for E2E.
...
- One shared skeleton for all 8 animated pairs, **`var(--rc-trans-dur)` (`2.4s`) `infinite`**:
...
- Panel colours: A = `#1f4a63` (teal-blue), B = `#63304a` (plum), container bg = `#000000`.
...
**The live preview card IS the drag handle** — no separate chip/thumbnail is dragged. Made `draggable` only while composing a brand-new card (`selectedCardId === null && text.trim() !== ""`). `onDragStart` sets `dataTransfer.setData("application/x-rushcut-card", "1")`, reusing the native HTML5 DnD mechanism `StickyFilmStrip` already had for dragging a trimmed cut in from TrimBar (#9) — a second mime type on the same drop-target handlers, not a parallel drag system.

**Repositioning a placed card (#151)** — a `CardStripTile` on the strip is itself `draggable` (Cards tab only, gated on `onRepositionCard` being passed). Dragging it to a different gap reuses the *same* blue insertion-line indicator and geometry as placing a new card; `onDragStart` tags the dataTransfer with `application/x-rushcut-card` **plus** `application/x-rushcut-card-id` (the card's id), which is how `onDropClipRow` tells "move this card" from "place a new one". The origin tile dims to `opacity: 0.45` while dragging (applied one frame *after* `dragstart` via `requestAnimationFrame` — mutating a `draggable` node's style during `dragstart` aborts the drag in WebView2; see LEARNINGS.md Tauri tag). Drop onto the card's own current gap = silent no-op; drop onto a gap held by a *different* card = rejected (red line), same as new-card placement. Cursor: `cursor-grab`. `selectedCardId` / the Cards-tab form mode are untouched by a move.
