...
3. **#110** — implement `hevc_amf` for final encode (was #85, closed GO 2026-07-11 — see below).
4. **#106 (P3-Low)** — copy-stream normalise for already-compliant clips. Narrow: never fires on the primary DJI HEVC workload.

`t_trim_s` has no remaining candidate after #104 closed NO-GO (below) — #99's parallelism win (~1.2x) is the ceiling for that stage barring a fundamentally different approach. Remaining north-star gap is `t_render_s` (63–69% of total) — #101/#88/#110 above all target it.
