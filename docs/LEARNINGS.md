...
**Problem:** #100 reported `t_normalise_s=25.0s` on a full proxy-skip render (19 clips, all clips reused a cached 2160p proxy) where `normalise()` is expected to be skipped entirely — 10x higher than the expected ~2-5s. Root cause, confirmed from `render.py`'s timing instrumentation (no render needed): `t0 = time.time()` (`render.py:469`) is set *before* the fps probe, the render-cache signature check, and the entire B-0 pre-trim `ThreadPoolExecutor` block (`render.py:540-547`); `normalise_s = time.time() - t0` (`render.py:603`) is computed *after* that block. On a full proxy-skip render `normalise()` itself is never called (falls to the `else: report(50)` branch, `render.py:600-601`), so the "normalise" timer is actually measuring B-0 pre-trim wall time almost entirely — the label doesn't match what's being timed.
**Solution:** Not a bug — the pipeline was behaving correctly, the timing label was misleading. No code change made; closed as a diagnosis. When reading `render-timing-log.jsonl`, do not assume `t_normalise_s` measures only `normalise()` encode work — on proxy-heavy renders it's dominated by B-0 pre-trim instead. WSL2 per-process spawn/init overhead is a plausible contributor to B-0's own cost regardless of it being a stream-copy (`-c copy`) rather than a re-encode — consistent with #104's finding that CPU-starvation and NTFS/tmpfs I/O-contention were both ruled out for Step 2 trim's fixed floor, leaving process-spawn/init overhead as the standing explanation for *why* short per-clip FFmpeg jobs have a floor at all, independent of encode type. Not independently re-verified for B-0's stream-copy case specifically — no per-clip B-0 timer exists yet, so the exact floor magnitude for a stream-copy job is still unmeasured (see #104 for the cross-reference).
**Context:** Any future timing-log analysis or "why is stage X slow" investigation on a proxy-skip/proxy-heavy render. Check what a `t0`/timer pair actually brackets in the source before trusting the printed label — a timing window can silently absorb unrelated setup work (fps probe, cache checks, per-clip pre-trim loops) that has nothing to do with the labeled stage. Confirmed #100, 2026-07-11.
