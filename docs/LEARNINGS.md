...
**AMF yuv444p negotiation — terminal `format=yuv420p` is insufficient when inputs have mismatched color primaries (#64):** Adding `[vpix]format=yuv420p[vout]` at the filtergraph output tail locks the *buffersink pixel format* but does NOT prevent AMF requesting yuv444p when driven by color-metadata mismatch. Confirmed: seg_0 and seg_1 (both inputs have consistent color metadata) now AMF-succeed after the fix; seg_2 still triggers `swscaler -129: fmt:yuv420p csp:bt709 prim:reserved -> fmt:yuv444p`. The `prim:reserved` originates from `trim_7.mp4`, an AMF-encoded intermediate whose color primaries are unset. When xfade mixes `prim:unknown` (libx264 intermediate) + `prim:reserved` (AMF intermediate), the format negotiation inside xfade requests yuv444p upstream of the terminal format node. **Next fix:** add `,format=yuv420p` to the per-input canvas pre-scale: `[{i}:v]{canvas},format=yuv420p[sv{i}]` — locks yuv420p at every buffersrc before xfade sees the color metadata. Also consider adding `-colorspace bt709 -color_primaries bt709 -color_trc bt709` to all intermediate encode steps to prevent `prim:reserved` from appearing on AMF-encoded files.
