...
## Pipeline — U1g both-boundary (open+close) drift falls back to monolithic, which then OOMs and can crash WSL itself (#114)

**Problem:** A different trigger than the `/tmp`-cleared case above, same destination and much worse outcome. On a 20-clip 4K project with **both** `opening_transition` and `closing_transition` active, all 7 U1g batches and both boundary re-encodes (`idx=0 open`, `idx=6 close`) succeeded, but the post-encode frame-drift guardrail then fired: `[U1g] boundary-only open/close frames=4310 expected=4312 (base=4306 + oc_delta=6) drift=2 frame(s)` — real gain was ~2 frames/boundary vs. the formula's assumed ~3 (`oc_delta_expected = round(0.1*fps_f)` per boundary). The `RuntimeError` is caught by the same blanket `except Exception`, discarding all the already-successful segmented work and falling back to a full 20-input monolithic filter_complex. That OOM'd on `h264_amf` (`Cannot allocate memory`), retried on `libx264 -preset medium`, and THAT run drove WSL from ~11.3 GB available down to 89 MB / swap nearly full before being OOM-killed with no `ERROR:`/`DONE:` ever logged. WSL itself was left unstable afterward — a subsequent unrelated `wsl` command hit `Catastrophic failure / WSL/Service/E_UNEXPECTED` and needed `wsl --shutdown` to recover. User only saw a generic "Pipeline timed out -- check WSL2 is running."
**Solution:** Not fixed yet — tracked as issue #114 (P1). Single-boundary (open-only or close-only) is not implicated; only both-on. Root cause of the 2-vs-3-frame gap is unconfirmed — two hypotheses (the `round(0.1*fps_f)` assumption itself vs. a timebase-snap/compounding effect across two boundaries in one render) are logged on the issue, deliberately not chased further without frame-level instrumentation of each boundary individually.
**Context:** `pipeline/render.py` drift check ~line 1264-1279 (`oc_drift_frames > 1: raise RuntimeError`). If reproducing: confirm `free -m` before AND immediately after any both-open+close 4K U1g render on a >BATCH_SIZE project — the monolithic fallback's memory footprint is severe enough to threaten the whole WSL VM, not just fail the one render.
