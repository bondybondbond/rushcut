...
## Pipeline — parallelising short FFmpeg re-encode jobs hits a fixed per-invocation floor, not a CPU ceiling

**Problem:** #99 parallelised `trim.py`'s Step 2 per-clip trim with a `ThreadPoolExecutor` mirroring `normalise.py`'s proven pattern (`MAX_PARALLEL_TRIM = min(4, os.cpu_count())`, per-worker `-threads` cap). Expected ~3-4x speedup (matching `normalise.py`'s precedent); measured only ~1.1-1.3x across every configuration tried (4×2, 2×4, 3×2 workers×threads; tested on both 8-core and 16-core WSL allocations; tested with inputs on `/mnt/c` NTFS and pre-localised to tmpfs — none of these variables moved the result). Per-job wall time was consistent across all configs: roughly a **7-8s fixed floor** regardless of thread count or concurrency, plus a smaller component proportional to output duration (~1.4x realtime). Two hypotheses were tested and disconfirmed: (1) CPU thread-starvation from capping `-threads` too low — ruled out because raising WSL from 8→16 cores and re-testing barely changed per-job time; (2) WSL 9P/NTFS I/O contention from concurrent `/mnt/c` reads — ruled out because pre-copying inputs to tmpfs before trimming didn't help either.
**Solution:** Not yet fixed — filed as #104 (real fix candidate: avoid the full re-encode `trim.py`/#96 introduced; e.g. re-encode only the boundary GOPs at the trim points and stream-copy the untouched middle, instead of re-encoding the whole clip). The parallelism from #99 is harmless and ships (correct output, ~1.2x average win), but do not expect a `ThreadPoolExecutor` wrapper alone to deliver `normalise.py`-style gains on **short** per-invocation FFmpeg jobs — the fixed startup/probe/init cost of spawning many short-lived FFmpeg processes dominates the total, and no amount of thread/worker tuning touches that fixed cost. `normalise.py`'s wins were measured on **longer** per-clip encodes where the fixed cost is a much smaller fraction of the total — same pattern, different regime.
**Context:** Any future "parallelise this per-clip FFmpeg loop" change. Before assuming a `ThreadPoolExecutor` wrapper will deliver `normalise.py`-style gains, check whether the per-job work is short enough (a few seconds of output) that fixed FFmpeg process overhead could dominate — if so, measure the sequential per-job baseline time first; if it's not roughly proportional to output duration, parallelism alone won't fix it.
**Context:** Any future git history rewrite (accidentally committed binaries, secrets, large files) on this project. Confirmed 2026-07-10 stripping old `src-tauri/target/` blobs — reduced `.git/` from 843MB to 17.1MB with zero disruption to `main`'s commit hash or the working tree.
