...
**AMF yuv444p negotiation — requires BOTH per-input `format=yuv420p` AND color primaries normalization (#64, fully resolved):** Two-lever fix required; either lever alone is insufficient.
- *Lever 1 (terminal format node — necessary but not sufficient):* `[vpix]format=yuv420p[vout]` at the filtergraph tail locks the buffersink but does NOT prevent AMF requesting yuv444p when upstream inputs carry mismatched color primaries (`prim:reserved` vs `prim:unknown`). Segments whose inputs share consistent color metadata succeed; segments mixing primaries still trigger `swscaler -129`.
- *Lever 2 (per-input canvas format node — necessary but not sufficient alone):* Adding `,format=yuv420p` to the per-input canvas pre-scale (`[{i}:v]{canvas},format=yuv420p[sv{i}]`) in `build_batch_video_fc` and `build_filter_complex` (transitions.py) locks every buffersrc to yuv420p before xfade negotiation. This eliminates the format-driven yuv444p request but still fails when the upstream decoder reports `prim:reserved` — AMF sees that in codec negotiation before the format filter can act.
- *Lever 3 (color primaries at source — resolves it):* The actual `prim:reserved` source is **Rust proxy gen** (`generate_proxy_file_low_priority` / `generate_proxy_file_normal_priority` in lib.rs) encoding with h264_amf without explicit color primaries. `trim_7.mp4` (seg 2's prim:reserved input) was a B-0 pretrim (`-c copy`) of a proxy — so it inherited the proxy's `prim:reserved` verbatim. Fix: add `-colorspace bt709 -color_primaries bt709 -color_trc bt709` to the AMF args block in both Rust proxy functions. Also add the same three flags to `zoom.py`'s `apply_zoom()` libx264 command so zoom-cache files can't propagate inherited `prim:reserved` from proxy inputs.
- **Verification:** "segmented render complete (no fallback)" in the job log, confirmed after proxies dir was wiped and render fell through to libx264 normalise (clean color metadata). Definitive test with fresh bt709 proxies is the next render after bg proxy gen rebuilds.
- **Source identification rule:** `prim:reserved` always originates from an AMF-encoded file (h264_amf without explicit `-color_primaries`). Trace the `trim_N.mp4` / `render_in_N.mp4` inputs in the failing segment's ffmpeg command back to their source (proxy or zoom cache). If `encoder: Lavc... h264_amf` appears in the ffprobe stream metadata, fix the encode site that produced that file.
