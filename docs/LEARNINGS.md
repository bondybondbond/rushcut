...
**Solution:** Two-layer enforcement. (1) `UserPromptSubmit` (`enforce-skill-trigger.js`) pattern-matches trigger phrases, injects the soft instruction, AND "arms" a small per-session state file (`.claude/hooks/lib/skill-gate.js`). (2) `PreToolUse` (`enforce-skill-gate.js`, matcher `".*"` — every tool) checks that state file on every subsequent tool call and returns `hookSpecificOutput.permissionDecision: "deny"` for anything except `Skill`/`AskUserQuestion` until the matching `Skill` call actually runs, which clears the gate. This is the real choke point — the harness enforces the deny regardless of what the model decides. Verified end-to-end against the live harness (not just pipe-tested): manually arming the gate for this session's real `session_id` caused subsequent `Bash` tool calls — even read-only ones — to be genuinely blocked by the harness, and the attempt-based fail-open (5 denied attempts, or 10 minutes) correctly auto-cleared and let the next call through. One anomaly during testing: the very first live tool call issued in the same turn the gate was armed was NOT blocked (all subsequent calls were, reliably) — root cause not identified; noted here in case it recurs.
