...
## [Workflow: git-filter-repo needs `safe.directory` for WSL-mounted NTFS repo]

**Context:** Any future git history rewrite (accidentally committed binaries, secrets, large files) on this project. Confirmed 2026-07-10 stripping old `src-tauri/target/` blobs — reduced `.git/` from 843MB to 17.1MB with zero disruption to `main`'s commit hash or the working tree. (See the fuller git-filter-repo entry earlier in this file for the actual fix steps — this was a stray duplicate Context line left over from a prior edit, now given its own heading.)
