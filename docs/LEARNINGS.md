...
**Solution:** Not yet fixed — #104 (partial-GOP trim: re-encode only the boundary GOPs, stream-copy the untouched middle) is now a confirmed GO per the corrected floor understanding, and #107 (batch all clips into one `filter_complex` call, amortizing the encode floor across N clips) is the lower-risk alternative recommended to evaluate first. The parallelism from #99 is harmless and ships (correct output, ~1.2x average win), but a `ThreadPoolExecutor` wrapper alone cannot reduce the encode-floor itself — it can only add more workers each paying their own share of real encode work.
**Context:** Any future "parallelise this per-clip FFmpeg loop" change, or any attempt to explain away a stubborn per-job floor as "process overhead." Before accepting that framing, isolate whether the job actually does encode work — time a `-c copy`/remux equivalent under the same contention conditions first. If copy is much cheaper than encode under contention, the floor is encode-specific and can be reduced by doing less encode work (batching, partial re-encode), not by adding threads/workers.
