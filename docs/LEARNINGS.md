...
**AMF fallback log line is now generic — do not assume "#64" from the log alone (#86):** `_run_with_amf_fallback()` in `render.py` catches ANY `RuntimeError` from an AMF encode and retries on libx264. Historically the warning read `*** AMF FALLBACK (#64) ***`, which only describes the yuv420p→yuv444p swscaler negotiation failure documented above. **A second, unrelated failure mode exists (#86):** Windows ffmpeg.exe raising `Error opening output ...: Permission denied` when writing a U1g segment output file over the `\\wsl.localhost\Ubuntu-24.04\tmp\rushcut-render\<job_id>\u1g_seg_N.mp4` UNC path — a filesystem/path issue, not a color-metadata negotiation issue. Confirmed reproducing twice in one session (2026-07-05) on two different job IDs, same project. The log line no longer says `(#64)` — it's just `*** AMF FALLBACK ***`. **Always read the actual ffmpeg stderr above the warning** to tell which failure mode occurred before assuming it's #64. #86 tracks the permission-denied case; suspected root cause is a U1g segment output path that isn't NTFS-backed the way the Batch U4c fix (below) intended.
