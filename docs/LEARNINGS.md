...
## [Workflow: Memory files are a soft bias, not enforcement — use a `UserPromptSubmit` hook for hard skill-trigger dispatch]

**Problem:** On issue #75, the user's opening message contained the literal trigger phrase "dev plan" for the `rushcut-dev-plan` skill, but it was treated as generic instruction text and answered with an ad-hoc manual Plan Mode flow (Explore/Plan agents, hand-written plan file) instead of calling the Skill tool. The output looked equivalent, but silently skipped the skill's own step machinery — specifically the auto-invoked `rushcut-qa-reviewer` step, which only fires from inside the real skill. A memory file was written after the fact to flag this, but the user correctly pointed out that a memory entry is a recall bias the model can still miss next time, not a guarantee.
**Solution:** For any "when the user says X, you must always do Y" rule that needs real enforcement (not just a reminder), configure a `UserPromptSubmit` hook (`.claude/hooks/*.js` + wiring in `.claude/settings.json`) that pattern-matches the trigger phrase in the incoming prompt and injects a mandatory `hookSpecificOutput.additionalContext` instruction via the harness — before the model does any reasoning. This runs independent of what the model "remembers." See `.claude/hooks/enforce-skill-trigger.js` for the concrete implementation (matches `rushcut-dev-plan`/`rushcut-wrapup` trigger phrases case-insensitively).
**Context:** Any time a project rule takes the shape "when the user's message contains phrase X, action Y must always happen first" — memory files and CLAUDE.md instructions are necessary but not sufficient; escalate to a hook when the cost of a miss is high (e.g. skipping an automated QA/verification step).
