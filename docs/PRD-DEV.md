...
- GPU encode — Batch Q (next). Windows `h264_amf` confirmed working (1-frame `AMF_OK` on AMD GPU). WSL2 path dead (no `/dev/dri`, no Linux AMF). Architecture: route Step 5/6 final encode through Windows `ffmpeg.exe` via `\\wsl.localhost\...` paths.
