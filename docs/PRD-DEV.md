...
| ------- | ---------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 6.0     | 2026-06-17 | Batch V1.1 DONE: Phantom clips + pantry undercount (#3 + #4 closed). Root cause: `pantryClips` decoration block in `Trimmer.tsx` spread `include: 1` onto source clip objects; `saveCurrentClip` persisted this via `update_clip_review_cmd`, flipping source template rows to `include=1`. Flipped rows disappeared from the pantry (`sourceClips = filter(include=0)`) and appeared at the front of the film strip (low scan-era sort_order). Fix: removed decoration block; `MediaPantry` receives undecorated `sourceClips` + explicit `inFilmPaths: Set<string>` prop (badge reads set membership, not `clip.include`); `saveCurrentClip` re-reads canonical `include` from top-level clips array. Repaired 2 flipped rows in Stagecoach 2025 project (inserted missing `include=0` source templates). 'All Clips' pantry header renamed to 'All Files'. Rule added to `rust-tauri.md`: `update_clip_review_cmd` is trim/zoom/focal save only — never `include` state. 9/9 fast + 12/12 trimmer PASS. |
| 5.9     | 2026-06-15 | Batch U6 + follow-up DONE: Music seek + loop toggle (issues #17 + #18 closed). "Loop music to fill film" toggle in Sound > Music tab (default ON, back-compat via `rc_sound_*`); `ma.loop = sound.musicLoop`; mute-bridge backward-seek (muted before `currentTime`, unmuted in one-shot `seeked` listener, 100ms skip tolerance). `music_loop` wired: `buildJobConfig.ts` → `types/project.ts` → `run.py` → `render.py` → `music.py` (loop=False forces n_copies=1; amix `duration=longest`). Follow-up NOW fixes: Bug A — `filmPlayingRef.current` guard at top of `advanceFilmClipRough` + active-slot check in both `onEnded` handlers (prevents spurious video-only extra clip after film-end); Bug B — idle click-catcher overlay (`absolute inset-0 z-10 cursor-pointer`) renders only while idle, calls `startFilmPlayback`, unmounts on play. PRD: sound-mixer decision point added. Issues #51–55 filed as U6a deferred; #54 = rewind-after-end no music (known root cause). DESIGN.md: idle click-catcher overlay pattern + z-ordering. LEARNINGS.md: `gh project item-add` silently fails (use GraphQL); stopped-at-end music restart. 9/9 fast PASS. |
| 5.8     | 2026-06-15 | Batch V1.2 DONE: WebView2 crash + driver reset bundle. #30 stale (swipe-delete gone since U2). #5 hardened: `playbackFailed` state in `Render.tsx` — onError when `videoLoadedRef=true` shows inline "Open in system player" fallback panel; closed. #29 Trimmer fixed: proxy-progress listener guarded with `sourceFailedRef` — only adopt proxy into selectedClip on recovery path, never for healthy clips. #29 Arrange: `diagLog` instrumentation + selectedClipId->null tracker; null-reset not reproduced, issue stays open. New issues: #48 filmstrip pan tooltip (V2.3/P3), #49 4K bitrate gap (V4.2/P2), #50 zoom pre-cache timing (V4.1/P2). 9/9 fast + 12/12 trimmer + 14/14 render PASS. |
