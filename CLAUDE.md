...
- **Repo is PUBLIC on GitHub (since 2026-07-12).** Never commit secrets, credentials, or personal paths — including in commit **messages**, not just file diffs (a message quoting a value being scrubbed elsewhere re-leaks it; see `docs/LEARNINGS.md` "Workflow — a commit message describing a secret/PII purge..."). `.claude/` is gitignored/untracked — keep it that way, never re-add it.
