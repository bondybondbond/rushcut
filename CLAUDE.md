...
- **`Skill(rushcut-wrapup)` is a hard gate (2026-07-15) — no session ends without it, including docs-only/probe/investigation sessions.** **Rewritten 2026-07-24 for #156 (CPO/Consultant/CC model):** the mechanical block point moved from the `Skill(rushcut-wrapup)` call itself to the `git commit` step inside it — wrapup's own docs/cleanup/backlog-harvest steps are safe to run unconditionally, but `git commit` (`enforce-pp-wrapup-signoff.js`) requires the MOST RECENT completed `rushcut-cpo` spawn in the session to have rendered a literal `VERDICT: APPROVE` (Gate 4, wrap-readiness) — a claim of approval without that proof does not satisfy the gate. Implementation edits during Step 6 are separately gated by `enforce-pp-plan-gates.js` requiring `rushcut-pp-consultant` to prove Gate 2 (WebSearch diversity) + Gate 3 (ChatGPT breadth/depth cycles — migrated off Perplexity 2026-08-25) and `rushcut-cpo` to render Gate 3's own `VERDICT: APPROVE`. See `docs/agent_plan.md` for the full model.
...
- **Two instances share one DB.** User runs `src-tauri/target/debug/rushcut.exe` directly (always-on Vite dev server). WDIO (`wdio.conf.ts`) launches a separate process of the same binary. Both write to `%APPDATA%\rushcut\rushcut.db`. Never confuse their generated artifacts — WDIO renders show `instance=wdio` in the timing log. **Exception:** `wdio.qa.conf.ts` (#170) sets `RUSHCUT_DATA_DIR` + `WEBVIEW2_USER_DATA_FOLDER` and skips single-instance — fully isolated DB/profile/ports (:9223/:9516). See `.claude/rules/e2e.md`.
